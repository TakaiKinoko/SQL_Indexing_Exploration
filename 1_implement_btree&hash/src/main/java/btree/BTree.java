 ...
 * listEntriesInOrder and List were slightly changed by Fang Han in order to index into data
 */
public class BTree<K extends Comparable, V>
{
    ...
    //
    public void list(BTIteratorIF<K, V> iterImpl, List<pair.Pair<Integer, Integer>> data, BufferedWriter tableWriter) throws IOException{
        if (mSize < 1) {
            return;
        }
        ...
        listEntriesInOrder(mRoot, iterImpl, data, tableWriter);
    }
    ...
    // Return false if it is done
    //
    private boolean listEntriesInOrder(BTNode<K, V> treeNode, BTIteratorIF<K, V> iterImpl, List<pair.Pair<Integer, Integer>> data, BufferedWriter tableWriter) throws IOException {
        if ((treeNode == null) ||
        ...
        for (int i = 0; i < currentKeyNum; ++i) {
            listEntriesInOrder(BTNode.getLeftChildAtIndex(treeNode, i), iterImpl,data, tableWriter);

            keyVal = treeNode.mKeys[i];
            // TODO test
            //StdOut.println("key: " + keyVal.mKey + " Value: " + data.get((Integer)keyVal.mValue).getValue());
            tableWriter.write("key: " + keyVal.mKey + " Value: " + data.get((Integer)keyVal.mValue).getValue());
            tableWriter.newLine();
            ...
            if (i == currentKeyNum - 1) {
                listEntriesInOrder(BTNode.getRightChildAtIndex(treeNode, i), iterImpl, data, tableWriter);
            }
        }
